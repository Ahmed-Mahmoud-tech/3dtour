...
- **Input validation (zod)**: request bodies for auth, admin user/subscription/assign/access, project create, and the public message form are validated by `validateBody(schema)` ([utils/validate.js](server/src/utils/validate.js)) at the **route**, using schemas in [validators/schemas.js](server/src/validators/schemas.js). On failure it returns 400 with the offending field; on success `req.body` is the parsed/normalized data (emails trimmed+lowercased, ids 24-hex, `ownerId`/`employeeId` coerced to `null`) so those controllers no longer type-check. Nested tour CRUD (nodes/hotspots/signs) is **not** zod-validated — the Mongoose sub-schemas enforce structure there. Add a schema + `validateBody` when adding a new scalar-body route; don't reintroduce hand-rolled `typeof` checks.
- **Security headers (helmet)**: [index.js](server/src/index.js) uses `helmet` with a strict API CSP (`default-src/script-src/object-src 'none'`) that sandboxes anything served from `/uploads` (a stored HTML/SVG can't execute), `crossOriginResourcePolicy: 'cross-origin'` so the frontends can still embed media, production-only HSTS, and `no-referrer`. The CSP does **not** constrain the Next/Vite apps (they're separate origins) — don't loosen it to accommodate them.
- **Media ownership ledger**: uploaded files are tracked in the [Upload](server/src/models/Upload.js) collection (`url` unique → owning `project`). Upload handlers record a row ([utils/mediaBinding.js](server/src/utils/mediaBinding.js) `recordUpload`); every project write path calls `bindIncomingMedia(req.body, project._id, req.user._id)` which extracts all `/uploads/...` URLs and **rejects (400) any URL already owned by another tour**, claiming unowned/legacy ones — this is what stops staff attaching (or deleting) another project's media. `shared:true` rows (legacy media referenced by several tours) are never gated. `deleteUploadByUrl` also drops the row; deleting a project purges its rows. **After deploying, run `node scripts/backfill-uploads.mjs` once** to seed the ledger from existing projects (binds single-tour URLs, flags cross-tour ones `shared`) — until then, existing media is claimed lazily on first save.
...
- The media ledger binds each `/uploads/...` URL to **one** tour: attaching the same uploaded URL to a second project is rejected 400 (`belongs to another tour`). The client uploads a fresh file per use, so this only bites if you deliberately reuse a URL across tours (mark its `Upload.shared = true` if that's intended). Run `scripts/backfill-uploads.mjs` after first deploying the ledger so existing multi-tour media is flagged `shared` rather than getting claimed by whichever tour saves first.
- Request-body validation lives in [validators/schemas.js](server/src/validators/schemas.js) via zod; when a controller reads `req.body` for a zod-guarded route, the data is already coerced/normalized — don't re-validate or re-normalize (e.g. emails are already lowercased/trimmed).
