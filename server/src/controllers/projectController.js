...
export const addNode = async (req, res) => {
  try {
    ...
    if (!displayName || !panoramaUrl)
      return res.status(400).json({ message: 'displayName and panoramaUrl are required' });

    const id = nodeId();
    const node = { id, displayName, panoramaUrl, initialYawOffset: initialYawOffset || 0 };
...
export const updateNode = async (req, res) => {
  try {
    ...
    if (!project.nodes.has(nodeId)) return res.status(404).json({ message: 'Node not found' });

    const existing = project.nodes.get(nodeId);
    project.nodes.set(nodeId, { ...existing, ...req.body, id: nodeId });
    await project.save();
...
export const updateHotspot = async (req, res) => {
  try {
    ...
    if (idx === -1) return res.status(404).json({ message: 'Hotspot not found' });

    // Separate transition data from hotspot fields
    const { _transitionData, ...hotspotBody } = req.body;

    node.navigationHotspots[idx] = { ...node.navigationHotspots[idx], ...hotspotBody, id: hotspotId };
    project.nodes.set(nodeId, node);
...
export const deleteHotspot = async (req, res) => {
  try {
    ...
    node.navigationHotspots = node.navigationHotspots.filter((h) => h.id !== hotspotId);
    project.nodes.set(nodeId, node);
    await project.save();
