...
const ensureStaticPlayerReady = async () => {
  ...
  if (!fs.existsSync(PLAYER_INDEX)) {
    throw new Error(
      "Static player build did not produce dist-static/index.html",
    );
  }
};
...
const rewriteMediaUrls = (value, files) => {
  ...
  if (value && typeof value === "object") {
    for (const k of Object.keys(value))
      value[k] = rewriteMediaUrls(value[k], files);
    return value;
  }
  return value;
};
...
export const exportProject = async (req, res) => {
  try {
    ...
    if (missing.length) {
      console.warn(
        `Export ${project._id}: ${missing.length} media file(s) missing:`,
        missing,
      );
