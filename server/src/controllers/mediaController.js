...
import fs from "fs";
import { optimizeAudio } from "../utils/mediaOptimizer.js";
...
export const uploadPanoramaHandler = asyncHandler(async (req, res) => {
  if (!req.file) return res.status(400).json({ message: "No file uploaded" });
  // Use original uploaded file without optimization to preserve quality
...
export const uploadTransitionVideo = asyncHandler(async (req, res) => {
  ...
  // Return original video without optimization to preserve quality
...
export const streamVideo = asyncHandler(async (req, res) => {
  ...
  if (!STREAM_FOLDERS.has(safeFolder))
  ...
  const contentType =
    STREAM_TYPES[path.extname(safeFilename).toLowerCase()] || "video/mp4";
